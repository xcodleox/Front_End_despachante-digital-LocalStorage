...
import { Mensagem, Servico, Solicitacao, Usuario } from '../interfaces/padrão';
...
export class AppService {
  ...
  obterChat(solicitacaoId: number): Mensagem[] {
    const solicitacao = this.listarSolicitacoes().find(
      ...
      const servicosPadrao: Servico[] = [
        {
          ...
          descricao:
            'Realizamos todo o processo de transferência de propriedade de forma digital, rápida e segura. Inclui mudança de titularidade e atualização do CRLV.',
          valor: 350.0,
          camposExtras: [
            'Placa do veículo',
            'Renavam',
          ...
          imagem:
            'https://images.unsplash.com/photo-1644058986421-237d141a9a66?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx2ZWhpY2xlJTIwdHJhbnNmZXIlMjBkb2N1bWVudHN8ZW58MXx8fHwxNzYxNjcyMzM1fDA&ixlib=rb-4.1.0&q=80&w=1080',
        },
        {
          id: 2,
          nome: 'Licenciamento Anual (CRLV)',
          descricao:
            'Renovação do licenciamento anual do veículo com emissão do Certificado de Registro e Licenciamento de Veículo atualizado.',
          valor: 150.0,
          camposExtras: ['Placa do veículo', 'Renavam', 'Ano de exercício'],
          imagem:
            'https://images.unsplash.com/photo-1695285161214-404148e1d408?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjYXIlMjBsaWNlbnNlJTIwcmVnaXN0cmF0aW9ufGVufDF8fHx8MTc2MTY3MjMzNXww&ixlib=rb-4.1.0&q=80&w=1080',
        },
        {
          id: 3,
          nome: 'Regularização de IPVA Atrasado',
          descricao:
            'Quitação e regularização de débitos de IPVA atrasado, incluindo cálculo de multas e juros.',
          valor: 200.0,
          camposExtras: ['Placa do veículo', 'Ano do débito'],
          imagem:
            'https://images.unsplash.com/photo-1565453004733-4759d7c8f712?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkb2N1bWVudCUyMHZlcmlmaWNhdGlvbiUyMG9mZmljZXxlbnwxfHx8fDE3NjE2NzIzMzZ8MA&ixlib=rb-4.1.0&q=80&w=1080',
        },
        {
          id: 4,
          nome: 'Segunda Via de Documentos',
          descricao:
            'Emissão de segunda via do CRV (Certificado de Registro de Veículo) e CRLV (Licenciamento) em caso de perda, roubo ou dano.',
          valor: 180.0,
        ...
        },
        {
          ...
          descricao:
            'Processo de baixa definitiva do veículo junto ao Detran, necessário para veículos que serão sucateados ou exportados.',
          valor: 250.0,
        ...
        },
        {
          ...
          descricao:
            'Alteração de categoria do veículo (particular para aluguel, táxi, etc.) com toda a documentação necessária.',
          valor: 300.0,
          camposExtras: [
            'Placa do veículo',
            'Categoria atual',
  ...
  private criarAdminPadrao() {
    ...
    if (!userExiste) {
      ...
      console.log(
        'Pergunta de segurança: Qual o nome do seu primeiro animal de estimação?'
      );
  ...
  obterOuRegistrarUsuario(
    nome: string,
    email: string,
    tipo: 'admin' | 'user' = 'user'
  ): { usuario: Usuario; novo: boolean } {
    const usuarios = this.listarUsuarios();
    const usuarioExistente = usuarios.find((u) => u.email === email);
  ...
  atualizarUsuario(id: number, dadosAtualizados: Partial<Usuario>): boolean {
    ...
    if (index !== -1) {
      if (
        ...
        dadosAtualizados.email !== usuarios[index].email
      ) {
  ...
  atualizarStatus(
    id: number,
    novoStatus: 'Em andamento' | 'Pausado' | 'Finalizado'
  ) {
    const solicitacoes = this.listarSolicitacoes();
